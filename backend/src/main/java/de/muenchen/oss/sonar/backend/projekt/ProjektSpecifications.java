...
public final class ProjektSpecifications {
    ...
    /* default */ static final char LIKE_ESCAPE = '!';
    ...
    public static Specification<Projekt> matching(final ProjektFilter filter) {
        final List<Specification<Projekt>> specifications = new ArrayList<>();

        if (filter.projektnummer() != null) {
            final String pattern = toLikePattern(filter.projektnummer());
