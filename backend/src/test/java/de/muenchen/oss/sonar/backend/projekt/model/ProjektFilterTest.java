...
class ProjektFilterTest {
    ...
    class None {
        @Test
        void givenNoCriteria_thenAllCriteriaAreNull() {
            final ProjektFilter result = ProjektFilter.none();

            assertThat(result.projektnummer()).isNull();
            assertThat(result.abrechnungBeginn()).isNull();
            assertThat(result.abrechnungEnde()).isNull();
        }
    }
